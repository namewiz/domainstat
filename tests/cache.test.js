...
test("clearCache should empty the cache", async (t) => {
  ...
  assert.ok(logs.some(l => l.includes("[check] domain=")), "Should have performed a lookup");
  logs.length = 0;

  // 2. Second check should be a cache hit
  await check(domain, { verbose: true, logger });
  assert.ok(logs.some(l => l.includes("[check] cache hit")), "Should have hit the cache");
  logs.length = 0;
  ...
  assert.ok(logs.some(l => l.includes("[check] domain=")), "Should have performed a fresh lookup");
});
